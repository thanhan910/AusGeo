# Download all xlsx files from "https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/access-and-downloads/allocation-files" and save them as CSV files.
# These files detail the allocation of Mesh blocks to geographical areas.
# Mesh blocks are the smallest geographical areas defined by the Australian Bureau of Statistics (ABS).
...
data_directory = f'{root_directory}/mb-info'
...
for href in xlsx_hrefs:
    ...
    DFS[name] = pd.read_excel(href, dtype=str)
    time_1 = time.perf_counter()
    print(f'[{name}]', 'Excel downloaded', time_1 - time_0)
    # Make sure to wrap in " " if there are , in the item
    # df.to_csv(f'{data_directory}/{name}.csv', index=False, quoting=1)
...
# Add odd state codes and SA2 codes to the state and SA2 tables

DF_ODD_SA2 = DFS['DZN_SA2_2021_AUST'][['SA2_CODE_2021', 'SA2_NAME_2021', 'STATE_CODE_2021', 'STATE_NAME_2021', 'AUS_CODE_2021', 'AUS_NAME_2021', 'AREA_ALBERS_SQKM']].drop_duplicates()

DF_ODD_SA2 = DF_ODD_SA2[~DF_ODD_SA2['SA2_CODE_2021'].isin(DFS['SA2_2021_AUST']['SA2_CODE_2021'])]

DFS['SA2_2021_AUST'] = pd.concat([DFS['SA2_2021_AUST'], DF_ODD_SA2[['SA2_CODE_2021', 'SA2_NAME_2021', 'STATE_CODE_2021', 'STATE_NAME_2021', 'AREA_ALBERS_SQKM']].drop_duplicates()])

DF_ODD_STATE = DF_ODD_SA2[~DF_ODD_SA2['STATE_CODE_2021'].isin(DFS['STE_2021_AUST']['STATE_CODE_2021'])]
DF_ODD_STATE.drop_duplicates(inplace=True)

DFS['STE_2021_AUST'] = pd.concat([DFS['STE_2021_AUST'], DF_ODD_SA2[['STATE_CODE_2021', 'STATE_NAME_2021', 'AUS_CODE_2021']].drop_duplicates()])
...
DF_CHANGE_FLAGS.dropna(inplace=True, subset=['CHANGE_FLAG_2021'])
